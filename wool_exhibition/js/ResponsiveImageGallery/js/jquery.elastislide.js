(function( window, $, undefined ) {
	...
	$.elastislide.defaults 		= {
		...
		imageW		: 70,	// the images width
		margin		: 15,	// image margin right
		border		: 4,	// image border
		minItems	: 6,	// the minimum number of items to show. 
	...
	$.elastislide.prototype 	= {
		...
		_setDim				: function( elW ) {
			...
			this.$items.css({
				...
				width		: ( elW ) ? Math.ceil(elW) : this.options.imageW + 2 * this.options.border //DMY round up
		...
		_setCurrentValues	: function() {
			...
			this.sliderW		= this.itemW * newCount;
		...
		_addControls		: function() {
			...
			this.$navNext	= $('<span class="es-nav-next tungsten-bold">&raquo;</span>');
			this.$navPrev	= $('<span class="es-nav-prev tungsten-bold">&laquo;</span>');
			$('<div class="es-nav"/>')
			.append( this.$navPrev )
			.append( this.$navNext )
			.appendTo( this.$el );
		...
		_slide				: function( dir, val, anim, callback ) {
			...
			if( val === undefined ) {
				...
				//var amount = this.itemW; // advance by single image DMY
				...
				if( dir === 'right' && this.sliderW - ( Math.abs( ml ) + amount ) < this.visibleWidth ) {
					...
					if( fml < this.sliderW - this.visibleWidth) //DMY -2 to ensure
						this._toggleControls( 'right', 1 );
					else	
						this._toggleControls( 'right', -1 );
		...
		_slideToCurrent		: function( anim ) {
			...
			// how much to slide?
			var amount	= this.current * this.itemW; // DMY -2
